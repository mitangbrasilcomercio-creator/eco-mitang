...
const outputFile = path.join(rootDir, 'WORKFLOW_ECOSYSTEM.md');

// Obter últimos commits para rastreabilidade
let lastCommitInfo = 'Desenvolvimento Inicial';
try {
  lastCommitInfo = execSync('git log -1 --pretty=format:"%h - %s (%cd)" --date=format:"%d/%m/%Y %H:%M"', { cwd: rootDir }).toString();
} catch (e) {}

const markdownContent = `# 🗺️ ECO-MITANG ERP: FLUXOGRAMA DINÂMICO DO ECOSSISTEMA & WORKFLOWS

> **Última Atualização Automática:** ${new Date().toLocaleString('pt-BR')}  
> **Último Commit:** \`${lastCommitInfo}\`  
...
console.log(`[OK] Fluxograma do ecossistema gerado com sucesso em: ${outputFile}`);
